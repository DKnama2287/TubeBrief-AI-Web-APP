"use client";

import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog";
import Image from "next/image";
import { Zap } from "lucide-react";
import { signIn } from "next-auth/react";
import { ReactNode } from "react";

export default function LoginModal({ children }: { children: ReactNode }) {
  return (
    <Dialog>
      <DialogTrigger asChild>{children}</DialogTrigger>
      <DialogContent className="bg-white dark:bg-slate-900 dark:border-slate-800 sm:max-w-sm">
        <DialogHeader>
          <div className="mx-auto mb-2 overflow-hidden rounded-xl">
            <Image src="/images/icon_192.png" alt="TubeBrief AI" width={52} height={52} className="block rounded-xl dark:hidden" />
            <Image src="/images/icon_194.png" alt="TubeBrief AI" width={52} height={52} className="hidden rounded-xl dark:block" />
          </div>
          <DialogTitle className="text-center text-xl font-bold text-slate-900 dark:text-white">
            Welcome to TubeBrief AI
          </DialogTitle>
          <p className="text-center text-sm text-slate-500 dark:text-slate-400">
            Sign in to start summarizing YouTube videos with AI
          </p>
        </DialogHeader>

        <div className="mt-4 flex flex-col gap-3">
          <button
            onClick={() => signIn("google")}
            className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition-all hover:border-violet-300 hover:bg-slate-50 hover:text-violet-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:border-violet-600 dark:hover:bg-slate-700 dark:hover:text-violet-400"
          >
            <svg className="h-5 w-5" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05" />
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
            </svg>
            Continue with Google
          </button>
        </div>

        <div className="mt-4 flex items-center gap-2 rounded-lg bg-violet-50 px-4 py-3 dark:bg-violet-950/40">
          <Zap className="h-4 w-4 shrink-0 text-violet-600 dark:text-violet-400" />
          <p className="text-xs text-violet-700 dark:text-violet-400">
            Get <strong>50 free coins</strong> on sign-up — that&apos;s 5 free video summaries!
          </p>
        </div>
      </DialogContent>
    </Dialog>
  );
}
